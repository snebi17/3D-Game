export class WebGL {
			...
			if (isArray) {
				...
				for (let k = 0; k < size; k++) {
					...
					addToObject(
						uniforms,
						elementName,
						gl.getUniformLocation(program, elementName)
					);
				}
			} else {
	...
	static configureAttribute(
		gl,
		{ location, count, type, normalize = false, stride = 0, offset = 0 }
	) {
	...
	static createSampler(
		gl,
		{ sampler = gl.createSampler(), wrapS, wrapT, min, mag }
	) {
		if (wrapS) {
